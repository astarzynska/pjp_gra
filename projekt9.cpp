...
const float FPS = 60;
enum STATE { PLAY, PYTANIA, MENU, OVER, AUTOR, INST };
...
int main() {
	...
	float dx_gw = 2, dx_gw1 = 3, dx_gw2 = 2;
	int okno = 0;
	int skok = 0;
	int i = 0;
	int liczba1, liczba2, liczba = 0, poprawna, zla1, zla2;
	int dzialanie, odp, losuj = 1;
	int start = 0;
	int kometa_on=0;
	int bez_nast_kom = 0;
	...
	while (ok) {
		...
		/*	if (punkty % 7 == 0 && punkty != 0 && bez_nast3 == 0) {
			...
			if ((punkty % 4 == 0) && (punkty != 0) && (bez_nast_kom==0)) {
				x_kom = x_kom + 1;
				y_kom = y_kom + 2;
			...
			if (punkty % 5 == 0 && punkty != 0 && bez_nast4 == 0) {
				if (x_player < x_straz) {
					x_straz = x_straz - 3;
				}
		...
		if (skok == 1) {
			...
			if (dzialanie == 1) {
				al_draw_text(font, al_map_rgb(255, 255, 255), 400, 310, 0, " + ");
			}
			if (dzialanie == 2) {
				al_draw_text(font, al_map_rgb(255, 255, 255), 400, 310, 0, " - ");
			}
			...
			al_draw_textf(font, al_map_rgb(255, 255, 255), 380, 310, 0, "%i ", liczba1);
			al_draw_textf(font, al_map_rgb(255, 255, 255), 440, 310, 0, "%i =", liczba2);
			al_draw_textf(font, al_map_rgb(255, 255, 255), 440, 350, 0, "%i", dzialanie);
			...
			if (odp == 2) {
				...
				al_draw_textf(font, al_map_rgb(255, 0, 0), 140, 410, 0, "odp. A) %i ", poprawna);
				al_draw_textf(font, al_map_rgb(255, 0, 0), 440, 410, 0, "odp. B) %i ", zla1);
				al_draw_textf(font, al_map_rgb(255, 0, 0), 640, 410, 0, "odp. C) %i ", zla2);
			}
			else if (odp == 3) {
				al_draw_textf(font, al_map_rgb(255, 0, 0), 140, 410, 0, "odp. A) %i ", zla1);
				al_draw_textf(font, al_map_rgb(255, 0, 0), 440, 410, 0, "odp. B) %i ", zla2);
				al_draw_textf(font, al_map_rgb(255, 0, 0), 640, 410, 0, "odp. C) %i ", poprawna);
			}
			...
			al_clear_to_color(al_map_rgb(128, 0, 64));
			al_draw_text(font, al_map_rgb(255, 255, 255), 380, 10, 0, "Zlodziej Gwiazdek");
			al_draw_text(font, al_map_rgb(255, 255, 255), 420, 110, 0, "Menu:");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 210, 0, "1. Nowa gra");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 260, 0, "2. Instrukcja");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 310, 0, "3. O Autorze");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 360, 0, "ESC. Wyjscie");
			al_flip_display();
		}

		else if (STATE == OVER) {
			al_clear_to_color(al_map_rgb(128, 0, 64));
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 210, 0, "GAME OVER");
			al_flip_display();
			al_rest(2.0);
			ok = false;
		}
			...
			al_clear_to_color(al_map_rgb(128, 0, 64));
			al_draw_text(font, al_map_rgb(255, 255, 255), 380, 10, 0, "O Autorze:");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 210, 0, "Imie: Alicja");
			al_draw_text(font, al_map_rgb(255, 255, 255), 400, 260, 0, "Nazwisko: Starzynska");
			al_flip_display();
		}
		else if (STATE == INST) {
			al_clear_to_color(al_map_rgb(128, 0, 64));
			al_draw_text(font, al_map_rgb(255, 255, 255), 380, 10, 0, "Instrukcja: ");
			al_draw_text(font, al_map_rgb(255, 255, 255), 300, 210, 0, "Poruszanie: strzalki w prawo i lewo");
			al_draw_text(font, al_map_rgb(255, 255, 255), 300, 260, 0, "Likwidacja komety: strzalka w dol");
			al_draw_text(font, al_map_rgb(255, 255, 255), 300, 310, 0, "Likwidacja straznika: strzalka w gore");
			al_draw_text(font, al_map_rgb(255, 255, 255), 300, 360, 0, "Cel: zdobycie jak najwiekszej liczby gwiazdek");
			al_flip_display();
		}
	}
